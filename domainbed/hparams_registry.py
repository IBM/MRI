...
def default_hparams(algorithm, dataset):
    return {key: default_val for key, (default_val, _) in get_hparams(algorithm, dataset, 0).items()}


def random_hparams(algorithm, dataset, seed):
    return {key: random_val for key, (_, random_val) in get_hparams(algorithm, dataset, seed).items()}
